...
//! # use diesel::prelude::*;
//! # async fn run_example(conn: &mut diesel_async::AsyncPgConnection) -> diesel::QueryResult<usize> {
//! # diesel::table! { users (name) { name -> Text, } }
//! # #[derive(Insertable)]
...
impl<T, Action, 'query> ExecuteDsl<AsyncPgConnection> for CopyFromQuery<T, Action>
where
    T: CopyTarget + Send + 'query,
    Action: CopyFromExpression<T> + Send + 'query,
    <Action as CopyFromExpression<T>>::Error: From<diesel::result::Error> + std::error::Error + Send + Sync + 'static,
{
    fn execute<'conn, 'query_>(
        self,
        conn: &'conn mut AsyncPgConnection,
    ) -> <AsyncPgConnection as crate::AsyncConnectionCore>::ExecuteFuture<'conn, 'query_>
    where
        Self: 'query_,
    {
        AsyncExecuteCopyFromDsl::execute(self, conn)
    }
}

impl<T, Action, 'a> ExecuteDsl<&'a mut AsyncPgConnection> for CopyFromQuery<T, Action>
where
    T: CopyTarget + Send + 'static,
    Action: CopyFromExpression<T> + Send + 'static,
    <Action as CopyFromExpression<T>>::Error: From<diesel::result::Error> + std::error::Error + Send + Sync + 'static,
{
    fn execute<'conn, 'query>(
        self,
        conn: &'conn mut &'a mut AsyncPgConnection,
    ) -> <&'a mut AsyncPgConnection as crate::AsyncConnectionCore>::ExecuteFuture<'conn, 'query>
    where
        Self: 'query,
    {
        AsyncExecuteCopyFromDsl::execute(self, *conn)
...
// We intentionally do NOT implement QueryId for CopyFromQuery
// to avoid satisfying the blanket ExecuteDsl implementation
// in src/run_query_dsl/mod.rs, which would conflict with our
// specific Copy-protocol based implementation.
